		...
		skinButton : function(c) {
			c.mouseover(function() {
				...
				$(this).removeClass("ui-state-focus ui-state-active")
					...
					function(d) {
						if (d.keyCode === $.ui.keyCode.SPACE
								|| d.keyCode === $.ui.keyCode.ENTER
								|| d.keyCode === $.ui.keyCode.NUMPAD_ENTER) {
...
PrimeFaces.ajax = {
	...
	Request : {
		...
		send : function(e) {
			...
			var m = (e.global === true || e.global === undefined) ? true
					: false, b = null, f = null;
			if (e.onstart) {
		...
		.extend({
			...
			render : function() {
				...
				for (var a = 0; a < this.columns.length; a++) {
					...
					$(
							'<li class="ui-columntoggler-item"><div class="ui-chkbox ui-widget"><div class="ui-chkbox-box ui-widget ui-corner-all ui-state-default ui-state-active"><span class="ui-chkbox-icon ui-icon ui-icon-check"></span></div></div><label>'
			...
			check : function(b) {
				b.addClass("ui-state-active").children(".ui-chkbox-icon")
...
PrimeFaces.widget.InputTextarea = PrimeFaces.widget.BaseWidget
		.extend({
			...
			updateCounter : function() {
				...
				if (this.counter) {
					var b = this.cfg.maxlength - c, a = this.cfg.counterTemplate
							.replace("{0}", b);
					this.counter.html(a)
				}
			},
...
PrimeFaces.widget.SelectCheckboxMenu = PrimeFaces.widget.BaseWidget
		.extend({
			...
			renderItems : function() {
				...
				this.inputs
						.each(function() {
							var c = $(this), d = c.next(), e = c
									.is(":disabled"), g = c.is(":checked"), h = "ui-chkbox-box ui-widget ui-corner-all ui-state-default", i = "ui-selectcheckboxmenu-item ui-selectcheckboxmenu-list-item ui-corner-all";
							if (e) {
								h += " ui-state-disabled"
							}
							if (g) {
							...
							var b = g ? "ui-chkbox-icon ui-icon ui-icon-check"
									: "ui-chkbox-icon ui-icon ui-icon-blank", i = g ? i
									+ " ui-selectcheckboxmenu-checked"
									: i + " ui-selectcheckboxmenu-unchecked";
							var f = $('<li class="' + i + '"></li>');
							f
									.append(
			...
			updateToggler : function() {
				if (this.itemContainer.children(
						"li.ui-selectcheckboxmenu-item:visible").filter(
						".ui-selectcheckboxmenu-unchecked").length === 0) {
					this.check(this.togglerBox)
				} else {
					this.uncheck(this.togglerBox)
				}
...
PrimeFaces.widget.ContextMenu = PrimeFaces.widget.TieredMenu
		.extend({
			...
			show : function(g) {
				...
				var f = $(window), d = g.pageX, c = g.pageY, b = this.jq
...
PrimeFaces.widget.PanelMenu = PrimeFaces.widget.BaseWidget
		.extend({
			...
			expandTreeItem : function(c, a, b) {
				c.children(".ui-panelmenu-icon").addClass(
						"ui-icon-triangle-1-s");
				a.show();
				if (!b) {
			...
			collapseTreeItem : function(b, a) {
				b.children(".ui-panelmenu-icon").removeClass(
						"ui-icon-triangle-1-s");
				a.hide();
			...
			restoreState : function() {
				...
				if (c) {
					...
					for (var b = 0; b < this.expandedNodes.length; b++) {
						var a = $(PrimeFaces
								.escapeClientId(this.expandedNodes[b]));
				...
				this.items.mouseover(function(b) {
					...
					if (!a.hasClass("ui-state-highlight")) {
						...
						function(b) {
							...
							if (!c) {
								a.removeClass("ui-state-hover").addClass(
						...
						function() {
							...
							if (!c.is(":first-child")) {
								c.hide(a.cfg.effect, {}, "fast", function() {
									c.insertBefore(c.prev()).show(a.cfg.effect,
											{}, "fast", function() {
												a.saveState()
											})
								})
			...
			moveTop : function(b) {
				...
				this.items.filter(".ui-state-highlight").each(
						function() {
							...
							if (!c.is(":first-child")) {
								c.hide(a.cfg.effect, {}, "fast", function() {
									c.prependTo(c.parent()).show(a.cfg.effect,
											{}, "fast", function() {
												a.saveState()
											})
								})
			...
			moveBottom : function(b) {
				...
				this.items.filter(".ui-state-highlight").each(
						function() {
							...
							if (!c.is(":last-child")) {
								c.hide(a.cfg.effect, {}, "fast", function() {
									c.appendTo(c.parent()).show(a.cfg.effect,
											{}, "fast", function() {
												a.saveState()
											})
								})
		...
		.extend({
			...
			bindEvents : function() {
				...
				if (this.cfg.showEvent === this.cfg.hideEvent) {
					...
					var a = this.cfg.showEvent + ".ui-overlaypanel", e = this.cfg.hideEvent
							+ ".ui-overlaypanel";
				...
				f.target.off("keydown.ui-overlaypanel keyup.ui-overlaypanel")
						.on("keydown.ui-overlaypanel", function(i) {
...
PrimeFaces.widget.TabView = PrimeFaces.widget.BaseWidget
		.extend({
			...
			remove : function(b) {
				var d = this.navContainer.children().eq(b), a = this.panelContainer
						.children().eq(b);
				this.fireTabCloseEvent(a);
				d.remove();
				a.remove();
				if (b < this.cfg.selected) {
					...
					if (b === this.cfg.selected) {
						var c = (this.cfg.selected === this.getLength()) ? this.cfg.selected - 1
								: this.cfg.selected;
						this.select(c, true)
					}
				}
...
PrimeFaces.widget.Tooltip = PrimeFaces.widget.BaseWidget
		.extend({
			...
			refresh : function(a) {
				if (a.target) {
					$(document.body).children(PrimeFaces.escapeClientId(a.id))
							.remove()
				} else {
...
PrimeFaces.widget.BaseTree = PrimeFaces.widget.BaseWidget
		.extend({
			...
			partialCheck : function(c) {
				var a = c.find("> .ui-chkbox-box > .ui-chkbox-icon"), b = c
						.closest(".ui-treenode"), d = this.getRowKey(b);
				this.removeFromSelection(d);
				b.find("> .ui-treenode-content > .ui-treenode-label")
						.removeClass("ui-state-highlight");
				...
				if (this.cfg.scrollHeight) {
					if (this.cfg.scrollHeight.indexOf("%") != -1) {
						var a = (this.jq.parent().innerHeight() * (parseInt(this.cfg.scrollHeight) / 100))
								- (this.scrollHeader.innerHeight() + this.scrollFooter
										.innerHeight());
						this.scrollBody.height(parseInt(a))
					}
				}
				var d = this;
				var c = $.browser.webkit ? "15px" : PrimeFaces
						.calculateScrollbarWidth();
			...
			fixColumnWidths : function() {
				...
				if (!this.columnWidthsFixed) {
					if (this.cfg.scrollable) {
						this.headerCols
								.each(function() {
									var f = $(this), c = f.index(), d = f
											.width(), b = f.innerWidth();
									f.width(d);
